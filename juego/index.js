...
class Juego {
  ...
  siguienteNivel() {
    if (this.nivel === 1) this.cambiarNivel();
      ...
      swal(
        ...
        `Siguiente nivel: ${this.nivel}, puntos: ${this.puntos}`,
        'success',
  ...
  perdioElJuego() {
    if (this.puntos > this.maxScore) {
      ...
      swal('Simon Dice', `Mejoraste tu puntuacion :)  ${this.puntos} `, 'success').then(() => {
        this.eliminarEventoClick();
        this.inicializar();
      });
    } else {
      swal('Simon Dice', `Lo lamentamos perdiste :(, puntos obtenidos:  ${this.puntos} `, 'gameover.svg').then(() => {
        this.eliminarEventoClick();
        this.inicializar();
      });
    }
